...
public class Block extends Actor {
    ...
    public Block(World world, Texture texture, Vector2 position) {
        ...
        def.position.set(position.x, position.y + 0.5f);                 // (2) Put the body in the initial position.
        ...
        fixture = body.createFixture(box, 1);       // (3) Create the fixture.
        fixture.setUserData("block1");              // (4) Set the user data.
        box.dispose();                              // (5) Destroy the shape.

        // Set the size to a value that is big enough to be rendered on the screen.
        setPosition((position.x - 0.5f) * Constants.PIXELS_IN_METER, position.y * Constants.PIXELS_IN_METER);
